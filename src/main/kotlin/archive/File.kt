...
fun openArchive(aFilePath: Path): ArchiveAndStream? {
...
private fun openSingleVolumeArchive(aFilePath: Path): ArchiveAndStream? {
...
private fun openMultiVolumeArchive(aFilePath : Path): ArchiveAndStream? {
...
class ArchiveOpenVolumeCallback : IArchiveOpenVolumeCallback, IArchiveOpenCallback {
    ...
    private val openedRandomAccessFileList = HashMap<Path, RandomAccessFile>()
    private var name: Path? = null
    ...
    @Throws(SevenZipException::class)
    override fun getStream(filename: Path): IInStream? {
        try {
